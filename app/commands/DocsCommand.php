 ...
 */
defined("BUILD_PATH") or define("BUILD_PATH", Yii::getPathOfAlias('application'));
...
class DocsCommand extends CConsoleCommand
{
    ...
    public $dressingOptions = array();
    public $boosterOptions = array();
...
EOD;
    ...
    public function run($args)
    {
        ...
        if (!isset($args[0]))
            $args[0] = Yii::getPathOfAlias('application.views.site.docs');
...
*/
        ...
        echo "Source URL : " . $this->dressingSourceUrl . "\n\n";
    ...
    public function renderSourceLink($sourcePath, $line = null)
    {
        ...
        if (file_exists(BUILD_PATH . $sourcePath)) {
            ...
            if ($line === null)
                return CHtml::link('dressing' . $sourcePath, $this->dressingSourceUrl . $sourcePath, array('class' => 'sourceLink'));
            else
                return CHtml::link('dressing' . $sourcePath . '#L' . $line, $this->dressingSourceUrl . $sourcePath . '#L' . $line, array('class' => 'sourceLink'));
        }
        else if (file_exists(Yii::getPathOfAlias('bootstrap') . $sourcePath)) {
            if ($line === null)
                return CHtml::link('booster' . $sourcePath, $this->boosterSourceUrl . $sourcePath, array('class' => 'sourceLink'));
            else
                return CHtml::link('booster' . $sourcePath . '#L' . $line, $this->boosterSourceUrl . $sourcePath . '#L' . $line, array('class' => 'sourceLink'));
        }
        else {
            if ($line === null)
                return CHtml::link('yii' . $sourcePath, $this->yiiSourceUrl . $sourcePath, array('class' => 'sourceLink'));
            else
                return CHtml::link('yii' . $sourcePath . '#L' . $line, $this->yiiSourceUrl . $sourcePath . '#L' . $line, array('class' => 'sourceLink'));
        }
    }
    ...
    protected function buildOfflinePages($docPath, $themePath)
    {
        ...
        if ($this->enableViews) {
            foreach ($this->views as $path => $view) {
                ...
                $content = preg_replace_callback(self::URL_PATTERN, array($this, 'fixOfflineLink'), $content);
                file_put_contents($docPath . '/' . $view->package . "." . $view->name . '.php', $content);
            }
        }
    ...
    protected function fixOfflineLink($matches)
    {
        if (($pos = strpos($matches[1], '::')) !== false) {
            ...
            return "<a href=\"{$className}.php#{$method}\">{$matches[2]}</a>";
        }
        else
            return "<a href=\"{$matches[1]}.php\">{$matches[2]}</a>";
    }
    ...
    protected function fixOnlineLink($matches)
    {
        if (($pos = strpos($matches[1], '::')) !== false) {
            ...
            $method = substr($matches[1], $pos + 2);
            if ($className === 'index')
                return "<?php echo CHtml::link('" . $matches[2] . "', array('/site/doc', '#' => '" . $method . "')); ?>";
            //return "<a href=\"/doc/api/#{$method}\">{$matches[2]}</a>";
            else
                return "<?php echo CHtml::link('" . $matches[2] . "', array('/site/doc', 'view' => '" . $className . "', '#' => '" . $method . "')); ?>";
            //return "<a href=\"/doc/api/{$className}#{$method}\">{$matches[2]}</a>";
        }
        else {
            if ($matches[1] === 'index')
                return "<?php echo CHtml::link('" . $matches[2] . "', array('/site/doc')); ?>";
            //return "<a href=\"/doc/api/\">{$matches[2]}</a>";
            else
                return "<?php echo CHtml::link('" . $matches[2] . "', array('/site/doc', 'view' => '" . $matches[1] . "')); ?>";
