<?php
/**
 * DocsModel represents the documentation for the application AND the Yii framework.
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: DocsModel.php 2854 2011-01-14 11:38:43Z mdomba $
...
class DocsModel
{
    ...
    public function build($sourceFiles, $viewFiles)
    {

        $this->findClasses($sourceFiles);

        $this->processClasses();
        $this->findViews($viewFiles);
        $this->processViews();
    }
    ...
    protected function findViews($sourceFiles)
    {
        $this->views = array();
        foreach ($sourceFiles as $sourceFile) {
            $this->views[$sourceFile] = file_get_contents($sourceFile);
        }
    ...
    {
        foreach ($this->views as $filename => $source) {
            $doc = $this->processView($filename, $source);
            $this->views[$doc->package . "." . $doc->name] = $doc;
            unset($this->views[$filename]);
            $this->packages[$doc->package][] = $doc->package . "." . $doc->name;
            if (isset($this->classes[$doc->controllerClass])) {
                $this->classes[$doc->controllerClass]->views[] = $doc->package . "." . $doc->name;
            }
        }
        ksort($this->packages);
    }

    protected function processView($filename, $source)
    {
        $doc = new ViewDoc();
        $doc->name = basename($filename, ".php");
        $path = explode(DIRECTORY_SEPARATOR, trim(dirname($filename), DIRECTORY_SEPARATOR));
        $folder = array_pop($path);
        $controllerClass = ucfirst($folder) . "Controller";
        $doc->controllerClass = $controllerClass;
        $doc->sourcePath = str_replace('\\', '/', str_replace(BUILD_PATH, '', $filename));
        $doc->package = "application." . str_replace("/", ".", trim(dirname($doc->sourcePath), "/"));
        $tokens = token_get_all($source);
        $inBlock = false;
        $comment = null;
        foreach ($tokens as $token) {
            if ($inBlock) {
                if ($token[0] === 367) { // doc comment
                    $comment = $token[1];
                    break;
                }
                elseif ($token[0] === 370) { // php close tag
                    break;
                }
            }
            elseif ($token[0] === 368) { // php open tag
                $inBlock = true;
            }
        }

        if ($comment !== null) {
            if (!isset($this->classes[$controllerClass])) {
                throw new CException('could not find ' . $controllerClass . ' for ' . $filename);
            }
            $this->processComment($doc, $comment);
            $doc->authors = $this->getDocAuthors($comment);
            $doc->uses = $this->getDocUses($comment, $this->classes[$controllerClass]);
            ksort($doc->uses);
    ...
    protected function processClass($class)
    {
        ...
        $doc->authors = $this->getDocAuthors($class->getDocComment());
        if ($doc->isFinal)
        ...
        if ($class->getName() === "Backlink") {
            file_put_contents("blah.txt", print_r($doc->properties, true));
        }
    ...
    protected function processProperties($class)
    {
        ...
        foreach ($this->getDocProperties($class) as $p) {
    ...
    protected function getDocProperties($class)
    {
        $comment = $class->getDocComment();
        preg_match_all('/(.*)@property (.*)/', $comment, $tags);
        $return = array();
        foreach ($tags[2] as $line) {

            $p = new PropertyDoc;
            $segs = explode(" ", str_replace("\t", " ", $line));

            $p->definedBy = $class->getName();
            $p->readOnly = false;
            $p->isStatic = false;
            $p->isProtected = false;
            $p->isInherited = false;

            $p->type = array_shift($segs);
            $p->name = trim(array_shift($segs), "$");
            $p->signature = "<b>" . $p->name . "</b>;";
            $p->signature = $this->getTypeUrl($p->type) . " " . $p->signature;
            /*$p->description = trim(implode(" ",$segs));*/
            $p->introduction = trim(implode(" ", $segs));
            $p->description = $p->introduction;
            /*if ($p->definedBy === "Backlink") {
                print_r($p->name);
    ...
    protected function getDocUses($comment, $class)
    {

        preg_match_all('/(.*)@uses (.*)/', $comment, $tags);
        $return = array();
        foreach ($tags[2] as $line) {
    ...
    protected function getDocAuthors($comment)
    {

        preg_match_all('/(.*)@author (.*)/', $comment, $tags);
        $return = array();
        foreach ($tags[2] as $line) {
            $return[] = trim($line);
    ...
    protected function tagProperty($object, $comment)
    {
        if ($object instanceof PropertyDoc) {
            $segs = preg_split('/\s+/', $comment, 2);
            $object->type = $segs[0];
...
class BaseDoc
{
    ...
    public function getSourceCode()
    {
        ...
        else if (file_exists(Yii::getPathOfAlias('dressing') . $this->sourcePath)) {
            $lines = file(Yii::getPathOfAlias('dressing') . $this->sourcePath);
        }
        else if (file_exists(Yii::getPathOfAlias('bootstrap') . $this->sourcePath)) {
            $lines = file(Yii::getPathOfAlias('bootstrap') . $this->sourcePath);
        }
        else {
